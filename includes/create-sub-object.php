...
$name_sub_object = $_POST["name-sub-object"];
...
$price = $_POST["price-object"];
$type_sub_object = $_POST["type-sub-object"];
$image_sub_object = $_FILES["image-sub-object"]["name"];
$image_sub_object_tmp = $_FILES["image-sub-object"]["tmp_name"];

$query_select_sub_object_check = "SELECT * FROM subobject WHERE SubObjectName = '$name_sub_object'";
$query_select_sub_object_check_result = mysqli_query($connection, $query_select_sub_object_check);
$query_select_sub_object_count = mysqli_num_rows($query_select_sub_object_check_result);

if($query_select_sub_object_count == 0)
{
    if(!$query_select_sub_object_check_result)
    {
        die("Query failed: " . mysqli_connect_error($connection));
    }
    else
    {
        $query_insert_sub_object = "INSERT INTO `subobject` (SubObjectName, SubObjectTypeId, ObjectId, Price, SubObjectImage, SubObjectLink) VALUES ('$name_sub_object_convert', $type_sub_object, $id_object, $price, '$image_sub_object', '$name_sub_object_link_to_lower_slash')";
        $query_insert_sub_object_result = mysqli_query($connection, $query_insert_sub_object);

        if(!$query_insert_sub_object_result)
        {
            die("Query failed: " . mysqli_connect_error($connection));
        }
        else
        {
            $select_sub_object = "SELECT * FROM subobject WHERE SubObjectLink = '$name_sub_object_link_to_lower_slash'";
            $select_sub_object_result = mysqli_query($connection, $select_sub_object);
            $select_sub_object_result_count = mysqli_num_rows($select_sub_object_result);

            if($select_sub_object_result_count > 0)
            {
                if(!$select_sub_object_result)
                {
                    die("Query failed: " . mysqli_connect_error($connection));
                }
                else
                {
                    $row = mysqli_fetch_assoc($select_sub_object_result);
                    $id_sub_object = $row["SubObjectId"];
                    $id_object = $row["ObjectId"];

                    $select_equipment = "SELECT * FROM equipment";
                    $query_equipment_result = mysqli_query($connection, $select_equipment);      

                    while($row_equip = mysqli_fetch_assoc($query_equipment_result))
                    {
                        $id_equip = $row_equip["EquipmentId"];

                        if(isset($_POST["$id_equip-equipment"]))
                        {
                            $insert_equipment_sub_object = "INSERT INTO `subobjequipment` (SubObjectId, EquipmentId) VALUES ($id_sub_object, $id_equip)";
                            $insert_equipment_sub_object_result = mysqli_query($connection, $insert_equipment_sub_object);

                            if(!$insert_equipment_sub_object_result)
                            {
                                die("Query failed: ". mysqli_connect_error($connection));
                            }
                        }
                    ...
                    $query_select_object_check = "SELECT ObjectLink FROM `object` WHERE ObjectId = $id_object";
                    ...
                    {
                        $row = mysqli_fetch_assoc($query_select_object_check_result);
                        $link_object = $row["ObjectLink"];

                        mkdir("objekti/$link_object$name_sub_object_link_to_lower/");
                        fopen("objekti/$link_object$name_sub_object_link_to_lower/index.php", "w");
                        move_uploaded_file($image_sub_object_tmp, "objekti/$link_object$name_sub_object_link_to_lower/$image_sub_object");

                        //SubObject nema samo jednu sliku vec ih ima vise, koje treba da se unesu u tabelu Photo
                        $file_content = fopen("objekti/$link_object$name_sub_object_link_to_lower/index.php", "w") or die("Unable to open file");
                        $index = "
                                <?php
                                                ...
                                                <div class='current-object-content'>
                                                    ...
                                                    <div class='content-address-button'>   
                                                        ...
                                                        <?php
                                                            ...
                                                            {
                                                                ?>
                                                                <button type='button' onclick='reserved(<?php echo \$id_sub_object; ?>);'>Reserve</button>
                                                                <?php
                                                            }
                                                        ?>
                                                        </div>
                                                    ...
                                                    <div class='wrapper-object-image'>
                                                        ...
                                                        \$query_select_photos = 'SELECT * FROM photo INNER JOIN subobject ON photo.SubObjectId = subobject.SubObjectId WHERE subobject.SubObjectId = $id_sub_object';
                                                ...
                                                <div class='wrapper-user-comments'>
                                                    ...
                                                    if(isset(\$_SESSION['id_user']))
                                                    {
                                                        ...
                                                        <div class='form-class'>
                                                            ...
                                                            <button type='button' onclick='leaveComment($id_sub_object);'>Send</button>
                                                    ...
                                                    <?php
                                                        ...
                                                        \$query = 'SELECT * FROM userobjectrating INNER JOIN user ON userobjectrating.UserId = user.UserId WHERE SubObjectId = $id_sub_object';
                        ...
                        fclose($file_content);
                        header("Location: makeoffer.php?operation=sub-post&id_object=$id_object");
                    }
                }
            }
            else
...
{
    echo "<p>The subjector already exists</p>";
}
