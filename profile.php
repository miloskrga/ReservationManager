            ...
            <div class="glavni" >
                <div style="width: 70%; position: relative; margin-left: auto; margin-right: auto; ">
                    <div class="personal-information">
                        <?php
                        $query_select_user_inf="SELECT * from `user` inner join city on user.CityId=city.CityId Where UserId=$id_user";
                        ...
                        if($query_select_user_inf_result_count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                $user_city=$row["CityId"]; 
                                $user_city_name=$row['CityName'];
                            }
                        }
                        ?>
                        <br><br>
                        ...
                        <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) ?>" >
                            ...
                            <div class="form-group">
                                <label for="city-name">City: <?php echo $user_city_name;?></label>
                            </div>
                            <div class="form-group">
                ...
                if(isset($_POST['submit'])){
                    ...
                    $query="UPDATE `user` SET FirstName='$fname', LastName='$lname', Email='$email', `Password`='$password', CityId=$city WHERE UserId=$id_user";
                    ...
                    { 
                        echo"<p style='text-align: center;'><b>You have successfully changed the data!</b></p>";
                    }
                }    
            ?>
            </div>
        ...
        <div class="container">
            <div style="max-width: 1100px; margin-left: auto; margin-right: auto; margin-top: 4rem;">
                ...
                <table class="table-reservation">
                    ...
                    <?php
                        //$query_select_reservation = "SELECT `reservation`.`ReservationId`,`subobject`.`SubObjectName`,`object`.`ObjectName`,`reservation`.`FromDate`,`reservation`.`ToDate`,`subobject`.`Price` FROM reservation INNER JOIN subobject ON reservation.SubObjectId = subobject.SubObjectId INNER JOIN `object` ON subobject.ObjectId = `object`.ObjectId WHERE reservation.UserId = $id_user";
                        $query_select_reservation="CALL GetReservation('".$id_user."')";
                        ...
                        else
                        {
                            while($row = mysqli_fetch_assoc($query_select_reservation_result))
                            {
                                ...
                                $tmp_date1=strtotime($date_from);
                                $from_date=date("d-m-Y",$tmp_date1);
                                $date_to = $row["ToDate"];
                                $tmp_date2=strtotime($date_to);
                                $to_date=date("d-m-Y",$tmp_date2);
                                $price = $row["Price"];
                                ...
                                <tr>
                                    ...
                                    <td><?php echo (int)($total_price=$date_of_days * $price); ?>&euro;</td>
                                    <?php
                    ...
                    ?>
                        <tr style="background-color: #605f5f;">
                            ...
                            <td style="padding: 0.5rem; color: #fff;"><?php echo $summary_number;?>&euro;</td>
                        </tr>
                    </tbody>
                </table>
